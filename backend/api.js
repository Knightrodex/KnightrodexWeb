...
exports.setApp = function (app, client) {
  ...
  app.post('/api/searchemail', async (req, res) => {
    ...
    try {
      ...
      if (requester == null) {
        response.error = 'Requesting User not found';
        res.status(500).json(response);
        return;
      }
    }
  ...
  app.post('/api/followuser', async (req, res) => {
    ...
    try {
      ...
      if (currentUser.usersFollowed.some(userId => userId.equals(otherUserId))) {
        response.error = 'User already follows other user';
        res.status(500).json(response);
      }
      else {
        userCollection.updateOne(
    ...
    try {
      ...
      for (const followedUserId of currUser.usersFollowed) {
        ...
        for (const badgeCollected of followedUser.badgesObtained) {
          ...
          const activityInfo = {
            email: followedUser.email, firstName: followedUser.firstName, lastName: followedUser.lastName, profilePicture: followedUser.profilePicture,
